...
Beispiel (Docker-Compose-Profil on-demand):
    docker compose --profile on-demand run --rm backtest python -m apps.champion_evals \
      ...
      --output /tmp/champion_evals.json
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    from apps.champion_evals.score import replay_ensemble, score_window, write_artifact
    ...
    if len(candles) < args.min_candles + args.horizon_bars:
        logger.error("Nicht genug Kerzen (%d) für min-candles=%d + horizon-bars=%d", len(candles), args.min_candles, args.horizon_bars)
        return 1
